...
class Server(object):
    ...
    def push_send(self, client, data):
        results_for_this_client = []
        print(self.client_async_results)
        if len(self.client_async_results) < 1:
            print('No queued results available for this client')
            client.send(json.dumps({'message': 'No results found in the queue '}).encode())
            return 
        # print(self.client_async_results)
        for result in self.client_async_results:
            try:
                if result['client'] == client:
                    results_for_this_client.append(result)
            except:
                traceback.print_exc()
                client.send(json.dumps({'message': 'No results found in the queue '}).encode())
                return
        ...
        count = 1
        result_queue = {}
        for q_res in  results_for_this_client: 
            server_result = {
                'time': q_res['time'],
                'func': q_res['func'],
                'res': q_res['res']
            }
            result_queue[count] = json.dumps(server_result)
            count += 1 
        client.send(json.dumps(result_queue).encode())
        time.sleep(10)
