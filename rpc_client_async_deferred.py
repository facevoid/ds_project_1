...
def get_push_message(s,id):
    ...
    push_recieve = s.recv(1024)
    if push_recieve:
        message = json.loads(push_recieve)
        print('\n')
        print(f"Push notification recieved for {message['func']}, made at {message['time']}")
        print('-' * 20)
        print(push_recieve)
        return
...
def calculate_pi(s):
    ...
    threading.Thread(target = get_push_message,args = (s, 0)).start()
    if len(response) < 2:
